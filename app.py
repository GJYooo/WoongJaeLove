...
with tab1:
    if db.empty:
        st.info("사이드바에서 '데이터 불러오기'를 먼저 눌러주세요.")
    else:
        num = st.number_input("문항 수", 1, len(db), min(10, len(db)), key="m_num")
        if st.button("🚀 새 시험 시작", key="m_start", use_container_width=True):
            ...
            st.session_state.total_solving_time = 0.0; st.session_state.q_start_time = time.time()
            st.rerun()
        ...
        if st.session_state.exam_list:
            ...
            if idx < len(exam):
                ...
                if u_in and not st.session_state.answered:
                    solve_time = time.time() - st.session_state.q_start_time
                    st.session_state.total_solving_time += solve_time
                ...
                if st.session_state.answered:
                    ...
                    with cn1:
                        if st.session_state.last_is_correct:
                            if st.button("🤔 내 생각과 다름 ➡️ 오답노트 추가", key=f"m_add_{idx}", use_container_width=True):
                                if q['문제'] not in st.session_state.wrong_notes['문제'].values:
                                    st.session_state.wrong_notes = pd.concat([st.session_state.wrong_notes, pd.DataFrame([q])], ignore_index=True)
                                    st.toast("추가 완료!")
                    with cn2:
                ...
                if st.button("시험 종료 및 초기화 🔄", use_container_width=True):
                    st.session_state.exam_list = []; st.rerun()
