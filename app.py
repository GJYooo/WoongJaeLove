...
with tab1:
    if db.empty:
        ...
        if st.button("🚀 새 시험 시작", key="mid_start", use_container_width=True):
            ...
            st.session_state.is_finished = False # 시험 종료 상태 추가
            st.rerun()
        ...
        if st.session_state.get('exam_list'):
            ...
            # [A] 문제 풀이 모드 (인덱스가 전체 문제 수보다 작을 때)
            ...
            # [B] 시험 결과 리포트 (인덱스가 끝에 도달했을 때)
                ...
                col_res3.metric("총 소요 시간", f"{total_time:.0f}초")
                
                col_res4, col_res5 = st.columns(2)
                col_res4.metric("문제당 평균 시간", f"{avg_time:.1f}초")
