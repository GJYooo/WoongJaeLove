...
with tab1:
    if db.empty:
        ...
        if st.button("🚀 새 시험 시작", key="mid_start", use_container_width=True):
            ...
            st.session_state.idx = 0
            st.session_state.answered = False
            st.session_state.correct_count = 0  # 맞은 개수 초기화
        ...
        if st.session_state.exam_list:
            ...
            if curr_idx < len(exam):
                ...
                if user_input:
                    ...
                    if user_input == "?":
                        ...
                        if is_correct:
                            st.session_state.correct_count += 1 # 맞은 개수 카운트
                            st.success("정답입니다! ✨")
                        else:
            ...
            # [B] 모든 문제를 다 풀었을 때 (통계 화면)
                ...
                avg_duration = total_duration / len(exam)
                accuracy = (st.session_state.correct_count / len(exam)) * 100
                
                # 깔끔한 대시보드 형태 출력
                m1, m2, m3 = st.columns(3)
                m1.metric("맞은 문제", f"{st.session_state.correct_count} / {len(exam)}")
                m2.metric("정답률", f"{accuracy:.1f}%")
                m3.metric("총 소요 시간", f"{total_duration:.0f}초")
                
                m4, m5 = st.columns(2)
                m4.metric("문제당 평균 시간", f"{avg_duration:.1f}초")
                m5.write("") # 공간 맞춤용
